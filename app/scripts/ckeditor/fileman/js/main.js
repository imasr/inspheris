...
function uploadFinished(e, i, res){
  ...
  if((httpRes && httpRes.res == 'error') || res != 'ok'){
    ...
    setUploadSuccess(i);
    if(httpRes.msg != ''){
    	  clickFirstOnEnter('msgDuplicateFile');
    	  $('#msg').html('Un fichier du même nom existe déjà, votre fichier sera importé sous la référence ' + httpRes.msg);
    	  var dialogButtons = {};    	  
    	  dialogButtons[t('OK')] = function(){$('#msgDuplicateFile').dialog('close');};
    	  $('#msgDuplicateFile').dialog({title: t('Message'),modal:true,buttons:dialogButtons});
    }    
  }
